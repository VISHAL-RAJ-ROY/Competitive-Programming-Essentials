...
gp<int,int> compress_The_Coordinates_1D( vi & v ) {
    sort(all(v)); auto it = unique(all(v)); v.resize(distance(v.begin(),it));
    gp<int,int> cor_comp; FOR(i,0,sz(v)) cor_comp[v[i]] = i;
    return cor_comp;
}
...
int main() {
    ...
    vi v{100,100,200,2000,300,10000,3022,4322,3022};
    gp<int,int> fg = compress_The_Coordinates_1D(v);
    error(fg);
    return 0;
}
