...
// o(n^2+m) implementation for dense graphs ( n^2 ~ m )
void dijkstra_for_dense_graphs( int s, ve<vii> & alist, vi & p, vi & d ) {
    int n = sz(alist); p.assign(n,-1); d.assign(n,oo);
    queue <int> vis;
    FOR(i,0,n) 
        vis.push(i); 
    d[s] = 0; int sd = n-1;
    FOR(i,0,n) {
        int v = vis.front();  vis.pop();
        FOR(j,0,sd) {
            int curr = vis.front(); vis.pop();
            if( d[curr] < d[v] ) 
                swap(v,curr);
            vis.push(curr);
        } sd--;
...
// o(nlog(m)+mlog(n)) 
void dijkstra_for_sparse_graphs_w_priority_queue( int s, ve<vii> & alist, vi & d, vi & p ) {
    int n = sz(alist);
    d.assign(n,oo);
    p.assign(n,-1);
    d[s] = 0;
    priority_queue<ii, vii, gr<ii>> pq;
    pq.push({0,s});
    while( !pq.empty() ) {
        ii top = pq.top(); pq.pop();
        int u = top.yy, dis = top.xx;
        if( dis != d[u] )   
            continue;
        SEL(ed,alist[u]) {
            int v = ed.xx, dist = ed.yy;
            if( d[v] > d[u]+dist ) 
                d[v] = d[u]+dist, p[v] = u, pq.push({d[v],v});
        }
    }
}

// o(nlog(m)+mlog(n))
// Has better performance than dijkstra_for_sparse_graphs_w_priority_queue
struct comp {
...
comp,
rb_tree_tag>
ordered_set;  // set from pbds can also be used 
vi d;
void dijkstra_for_sparse_graph_w_set_no_pair( int s, ve<vii> & alist, vi & p ) {
    int n = sz(alist); p.assign(n,-1); d.assign(n,oo);
    ...
    while( !ver.empty() ) {
        ...
        SEL(ed,alist[v]) {
            int u = ed.xx, dis = ed.yy; 
            if( d[u] > d[v]+dis ) 
                ver.erase(u), d[u] = d[v]+dis, p[u] = v, ver.insert(u);
        }
    }
}
// set using pair<int,int>
typedef tree<
int,
null_type,
less<ii>,
rb_tree_tag>
ordered_set;  // set from pbds can also be used 
void dijkstra_for_sparse_graph_w_set_w_pair(int s, ve<vii> &alist, vi &p)
{
    int n = sz(alist);
    p.assign(n, -1);
    d.assign(n, oo);
    set<ii> ver; // ordered_set ver;
    d[s] = 0;
    ver.insert({0,s});
    while (!ver.empty())
    {
        int v = ver.begin()->second;
        ver.erase(ver.begin());
        SEL(ed, alist[v])
        {
            int u = ed.xx, dis = ed.yy;
            if (d[u] > d[v] + dis)
                ver.erase({d[u],u}), d[u] = d[v] + dis, p[u] = v, ver.insert({d[u],u});
        }
    }
}
